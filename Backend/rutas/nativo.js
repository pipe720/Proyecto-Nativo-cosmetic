...
// POST - Iniciar sesión (Opción B: Híbrido para pruebas manuales en Compass)
router.post("/clientes/login", async (req, res) => {
    try {
        const { correo, contrasena } = req.body;

        // 1. Buscar si el cliente existe en MongoDB por su correo
        ...
        // Identificamos si la contraseña guardada en la BD es un hash encriptado
        const esUnHash = cliente.contrasena.startsWith("$2b$") || cliente.contrasena.startsWith("$2a$");

        if (esUnHash) {
            try {
                // Si es un hash, lo comparamos usando bcrypt
                esCorrecta = await bcrypt.compare(contrasena, cliente.contrasena);
            } catch (err) {
                // Salvavidas por si falla la librería
                esCorrecta = (contrasena === cliente.contrasena);
            }
        } else {
            // Si la escribiste a mano en Compass (ej: "123456"), la compara directamente en texto plano
            esCorrecta = (contrasena === cliente.contrasena);
        }
        ...
        // 3. Responder con la estructura que tu Frontend espera
        return res.status(200).json({
            ...
            usuario: {
                ...
                rol: cliente.rol || "usuario" // Ponle "admin" en Compass si quieres probar vistas de administrador
            }
        });
